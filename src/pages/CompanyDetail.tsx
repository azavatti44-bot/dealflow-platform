...
export default function CompanyDetail() {
  ...
  if (loading) return <div className="max-w-6xl mx-auto px-4 py-12 text-center"><Loader2 className="animate-spin inline mr-2" style={{ color: "#16A34A" }} /><span style={{ color: "#64748B" }}>Loading...</span></div>;
  if (!company) return <div className="max-w-6xl mx-auto px-4 py-12 text-center" style={{ color: "#64748B" }}>Company not found.</div>;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 space-y-4">
      <button onClick={() => navigate(-1)} className="flex items-center gap-1 text-xs" style={{ color: "#64748B" }}><ArrowLeft size={14} />Back</button>

      <div className="rounded-xl p-5" style={card}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="font-serif text-2xl font-bold" style={{ color: "#1A2416" }}>{company.name}</h1>
            <div className="flex items-center gap-3 mt-2 text-[10px] font-mono" style={{ color: "#64748B" }}>
              ...
              <span className="px-2 py-0.5 rounded-full font-bold" style={{ background: company.tier === "hot" ? "rgba(22,163,74,0.12)" : "rgba(0,0,0,0.04)", color: company.tier === "hot" ? "#16A34A" : "#64748B" }}>
          ...
          <div className="flex items-center gap-2">
            {watchlists.map(w => (
              <button key={w.id} onClick={() => toggleWatchlist(company.name, w.id)}
                ...
                style={{ background: inWl(w.id) ? "rgba(22,163,74,0.1)" : "rgba(0,0,0,0.04)", color: inWl(w.id) ? "#16A34A" : "#64748B" }}>
                <Star size={10} style={{ fill: inWl(w.id) ? "#16A34A" : "none" }} />{w.name}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              <div key={label} className="rounded-lg p-4 text-center" style={card}>
                <Icon className="mx-auto mb-1.5" size={16} style={{ color: "#16A34A" }} />
                <p className="text-xl font-bold font-mono" style={{ color: "#1A2416" }}>{value}</p>
          ...
          {company.signals.length > 0 && (
            <div className="rounded-xl p-4" style={card}>
              <h3 className="text-sm font-semibold mb-3" style={{ color: "#1A2416" }}>Detected Signals ({company.signals.length})</h3>
              {company.signals.map((s, i) => (
                <div key={i} className="flex items-start gap-3 py-2" style={{ borderBottom: i < company.signals.length - 1 ? "1px solid rgba(0,0,0,0.04)" : "none" }}>
                  <span className="w-2 h-2 rounded-full mt-1 shrink-0" style={{ background: s.confidence === "high" ? "#16A34A" : s.confidence === "medium" ? "#22C55E" : "#64748B" }} />
                  <div>
                    <p className="text-sm font-medium" style={{ color: "#1A2416" }}>{s.signal_type.replace(/_/g, " ")}</p>
      ...
      {tab === "signals" && (
        <div className="space-y-2">
          {company.signals.length === 0 && <p className="text-sm text-center py-8" style={{ color: "#64748B" }}>No signals detected.</p>}
          {company.signals.map((s, i) => (
            <div key={i} className="rounded-lg p-4" style={card}>
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded" style={{ background: s.confidence === "high" ? "rgba(22,163,74,0.1)" : s.confidence === "medium" ? "rgba(22,163,74,0.12)" : "rgba(0,0,0,0.04)", color: s.confidence === "high" ? "#16A34A" : s.confidence === "medium" ? "#22C55E" : "#64748B" }}>{s.confidence}</span>
                <span className="text-[10px] font-mono" style={{ color: "#64748B" }}>+{s.score_impact} pts</span>
              </div>
              <h4 className="text-sm font-semibold" style={{ color: "#1A2416" }}>{s.signal_type.replace(/_/g, " ")}</h4>
              ...
              <p className="text-[10px] mt-1" style={{ color: "#16A34A" }}>{s.source}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {tab === "filings" && (
        <div className="rounded-xl p-4" style={card}>
          {company.sec_filings.length === 0 && <p className="text-sm text-center py-8" style={{ color: "#64748B" }}>No filings found.</p>}
          <table className="w-full text-xs">
            <thead><tr style={{ borderBottom: "1px solid rgba(0,0,0,0.08)" }}>
              <th className="text-left py-2 px-2" style={{ color: "#64748B" }}>Form</th>
              <th className="text-left py-2 px-2" style={{ color: "#64748B" }}>Filed</th>
              <th className="text-left py-2 px-2" style={{ color: "#64748B" }}>Description</th>
              <th className="text-left py-2 px-2" style={{ color: "#64748B" }}>Items</th>
            </tr></thead>
            <tbody>{company.sec_filings.map((f, i) => (
              <tr key={i} style={{ borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
                <td className="py-2 px-2"><span className="px-2 py-0.5 rounded text-[10px] font-mono" style={{ background: "rgba(22,163,74,0.08)", color: "#16A34A" }}>{f.form_type}</span></td>
                <td className="py-2 px-2 font-mono" style={{ color: "#64748B" }}>{f.filed_at}</td>
                <td className="py-2 px-2" style={{ color: "#1A2416" }}>{f.description}</td>
      ...
      {tab === "court" && (
        <div className="rounded-xl p-4" style={card}>
          {company.court_cases.length === 0 && <p className="text-sm text-center py-8" style={{ color: "#64748B" }}>No court cases found.</p>}
          {company.court_cases.map((c, i) => (
            <div key={i} className="py-3" style={{ borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
              <div className="flex items-center gap-2 mb-1">
                <span className="px-2 py-0.5 rounded text-[10px] font-mono" style={{ background: c.case_type === "bankruptcy" ? "rgba(139,58,58,0.15)" : "rgba(0,0,0,0.04)", color: c.case_type === "bankruptcy" ? "#C9A0A0" : "#64748B" }}>{c.case_type}</span>
                <span className="text-[10px] font-mono" style={{ color: "#64748B" }}>{c.docket_number}</span>
              </div>
              <p className="text-sm" style={{ color: "#1A2416" }}>{c.case_name}</p>
              <p className="text-xs" style={{ color: "#64748B" }}>{c.court} · {c.date_filed}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {tab === "patents" && (
        <div className="rounded-xl p-4" style={card}>
          {company.patents.length === 0 && <p className="text-sm text-center py-8" style={{ color: "#64748B" }}>No patents found.</p>}
          {company.patents.map((p, i) => (
            <div key={i} className="py-2 flex items-start gap-3" style={{ borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
              <span className="font-mono text-[10px] px-2 py-0.5 rounded shrink-0" style={{ background: "rgba(74,124,89,0.1)", color: "#22C55E" }}>{p.number}</span>
              <div><p className="text-sm" style={{ color: "#1A2416" }}>{p.title}</p><p className="text-[10px] font-mono" style={{ color: "#64748B" }}>{p.date}</p></div>
            </div>
          ))}
        </div>
      )}
      ...
      {tab === "state" && (
        <div className="rounded-xl p-4" style={card}>
          {company.sos_records.length === 0 && <p className="text-sm text-center py-8" style={{ color: "#64748B" }}>No state records found.</p>}
          {company.sos_records.map((r, i) => (
            <div key={i} className="py-3" style={{ borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
              <p className="text-sm font-semibold" style={{ color: "#1A2416" }}>{r.entity_name}</p>
              ...
              {r.officers.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {r.officers.slice(0, 5).map((o, j) => (
                    <span key={j} className="text-[10px] px-2 py-0.5 rounded font-mono" style={{ background: "#F0EDE5", color: "#64748B" }}>{o.name} — {o.position}</span>
                  ))}
                </div>
              )}
            </div>
      ...
      {tab === "form_d" && (
        <div className="rounded-xl p-4 space-y-4" style={card}>
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold" style={{ color: "#1A2416" }}>Reg D Filings</h3>
            <button
          ...
          {formDData.length === 0 ? (
            ...
            <table className="w-full text-xs">
              <thead><tr style={{ borderBottom: "1px solid rgba(0,0,0,0.08)" }}>
                <th className="text-left py-2 px-2" style={{ color: "#64748B" }}>Company</th>
                <th className="text-left py-2 px-2" style={{ color: "#64748B" }}>State</th>
                <th className="text-left py-2 px-2" style={{ color: "#64748B" }}>Filed</th>
              ...
              <tbody>
                {formDData.map((r, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
                    <td className="py-2 px-2" style={{ color: "#1A2416" }}>{r.company_name}</td>
                    <td className="py-2 px-2 font-mono" style={{ color: "#64748B" }}>{r.state}</td>
                    <td className="py-2 px-2 font-mono" style={{ color: "#64748B" }}>{r.date_filed}</td>
                    <td className="py-2 px-2">
                      <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold"
                        style={r.form_type === "D/A"
                          ? { background: "rgba(90,77,58,0.3)", color: "#64748B" }
      ...
      {tab === "sba" && (() => {
        ...
        if (!sba) return (
          <div className="rounded-xl p-6 text-center" style={card}>
            <p className="text-sm" style={{ color: "#64748B" }}>No SBA PPP data found — company may be too large, too small, or did not participate</p>
        ...
        return (
          <div className="space-y-4">
            <div className="rounded-xl p-5" style={card}>
              <h3 className="text-sm font-semibold mb-4" style={{ color: "#1A2416" }}>SBA PPP Size Data</h3>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={d.label} className="rounded-lg p-3" style={{ background: "#F0EDE5" }}>
                    <p className={`text-sm font-bold ${d.mono ? "font-mono" : ""}`} style={{ color: "#1A2416" }}>{d.value || "—"}</p>
            ...
            <div className="rounded-xl p-5" style={card}>
              <h3 className="text-sm font-semibold mb-4" style={{ color: "#1A2416" }}>Estimated Financials</h3>
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="rounded-lg p-3" style={{ background: "#F0EDE5" }}>
                  <p className="text-sm font-bold font-mono" style={{ color: "#16A34A" }}>{fmt(revLow)} – {fmt(revHigh)}</p>
                  <p className="text-[10px] mt-0.5" style={{ color: "#64748B" }}>Revenue Estimate</p>
                </div>
                <div className="rounded-lg p-3" style={{ background: "#F0EDE5" }}>
                  <p className="text-sm font-bold font-mono" style={{ color: "#22C55E" }}>{fmt(ebitdaLow)} – {fmt(ebitdaHigh)}</p>
