...
const statusIcon = (s: string) => s === "operational" ? <Check size={11} style={{ color: "#34D399" }} /> : s === "degraded" ? <Clock size={11} style={{ color: "#f59e0b" }} /> : <X size={11} style={{ color: "#8B3A3A" }} />;
const statusColor = (s: string) => s === "operational" ? { bg: "rgba(74,124,89,0.12)", text: "#34D399" } : s === "degraded" ? { bg: "rgba(245,158,11,0.12)", text: "#f59e0b" } : { bg: "rgba(139,58,58,0.12)", text: "#8B3A3A" };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 space-y-4">
      <div>
        ...
        <p className="text-xs mt-1" style={{ color: "#94A3B8" }}>API configuration and preferences</p>
      </div>

      <div className="flex flex-wrap gap-1 pb-3" style={{ borderBottom: "1px solid rgba(148,163,184,0.12)" }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs transition-colors"
            style={tab === t.id ? { background: "#6EE7B7", color: "#0B1A0E", fontWeight: 600 } : { color: "#94A3B8" }}>
          ...
          {APIS.map(api => (
            <div key={api.name} className="rounded-xl p-4" style={card}>
              <div className="flex flex-col md:flex-row md:items-center gap-3">
                <div className="p-2 rounded-lg shrink-0" style={{ background: "#1A351F" }}><api.icon size={16} style={{ color: "#6EE7B7" }} /></div>
                <div className="flex-1">
                  ...
                  <p className="text-[11px] mt-1" style={{ color: "#94A3B8" }}>{api.desc}</p>
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {api.endpoints.map(e => <span key={e} className="text-[9px] px-2 py-0.5 rounded font-mono" style={{ background: "#1A351F", color: "#64748B" }}>{e}</span>)}
                  </div>
                </div>
                <div className="text-right shrink-0"><p className="text-[9px]" style={{ color: "#64748B" }}>Latency</p><p className="text-sm font-mono" style={{ color: "#F5F0E6" }}>{api.latency}</p></div>
              </div>
            </div>
          ))}

          <div className="rounded-xl p-4" style={{ background: "rgba(110,231,183,0.05)", border: "1px solid rgba(110,231,183,0.15)" }}>
            <div className="flex items-center gap-2 mb-1"><Shield size={14} style={{ color: "#6EE7B7" }} /><h3 className="text-sm font-semibold" style={{ color: "#F5F0E6" }}>How It Works</h3></div>
          ...
          {notifs.map(n => (
            <div key={n.id} className="rounded-xl p-4 flex items-center justify-between" style={card}>
              ...
              <button onClick={() => setNotifs(prev => prev.map(x => x.id === n.id ? { ...x, on: !x.on } : x))}
                className="w-9 h-5 rounded-full relative transition-colors" style={{ background: n.on ? "#34D399" : "rgba(255,255,255,0.1)" }}>
      ...
      {tab === "account" && (
        <div className="space-y-4 max-w-md">
          <div className="rounded-xl p-4 flex items-center gap-3" style={card}>
            <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold" style={{ background: "rgba(110,231,183,0.2)", color: "#6EE7B7" }}>ST</div>
            <div><p className="font-semibold text-sm" style={{ color: "#F5F0E6" }}>Saint Thomas Capital Partners</p><p className="text-[10px]" style={{ color: "#94A3B8" }}>Signal Engine — Internal</p></div>
          </div>
          <div className="rounded-xl p-4 space-y-3" style={card}>
            <div>
              ...
              <input defaultValue="Saint Thomas Capital Partners" className="w-full px-3 py-2 rounded-lg text-sm outline-none"
                style={{ background: "#1A351F", color: "#F5F0E6", border: "1px solid rgba(148,163,184,0.1)" }} />
            </div>
            <div>
          ...
          <button className="w-full py-2 rounded-lg text-sm font-semibold" style={{ background: "#6EE7B7", color: "#0B1A0E" }}>Save Changes</button>
        </div>
      )}
    </div>
  );
