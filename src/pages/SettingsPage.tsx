...
const statusIcon = (s: string) => s === "operational" ? <Check size={11} style={{ color: "var(--accent-hover)" }} /> : s === "degraded" ? <Clock size={11} style={{ color: "#f59e0b" }} /> : <X size={11} style={{ color: "#DC2626" }} />;
const statusColor = (s: string) => s === "operational" ? { bg: "rgba(27,67,50,0.12)", text: "var(--accent-hover)" } : s === "degraded" ? { bg: "rgba(245,158,11,0.12)", text: "#f59e0b" } : { bg: "rgba(220,38,38,0.1)", text: "#DC2626" };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 space-y-4">
      <div>
        ...
        <p className="text-xs mt-1" style={{ color: "var(--text-secondary)" }}>API configuration and preferences</p>
      </div>
      ...
      <div className="flex flex-wrap gap-1 pb-3" style={{ borderBottom: "1px solid rgba(0,0,0,0.08)" }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            ...
            style={tab === t.id ? { background: "var(--accent)", color: "#FFFFFF", fontWeight: 600 } : { color: "var(--text-secondary)" }}>
      ...
      {tab === "apis" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>API Integration Status</h2>
          ...
          {APIS.map(api => (
            <div key={api.name} className="rounded-xl p-4" style={card}>
              <div className="flex flex-col md:flex-row md:items-center gap-3">
                <div className="p-2 rounded-lg shrink-0" style={{ background: "var(--bg-surface-alt)" }}><api.icon size={16} style={{ color: "var(--accent)" }} /></div>
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>{api.name}</h3>
                  ...
                  <p className="text-[11px] mt-1" style={{ color: "var(--text-secondary)" }}>{api.desc}</p>
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {api.endpoints.map(e => <span key={e} className="text-[9px] px-2 py-0.5 rounded font-mono" style={{ background: "var(--bg-surface-alt)", color: "var(--text-secondary)" }}>{e}</span>)}
                  </div>
                </div>
                <div className="text-right shrink-0"><p className="text-[9px]" style={{ color: "var(--text-secondary)" }}>Latency</p><p className="text-sm font-mono" style={{ color: "var(--text-primary)" }}>{api.latency}</p></div>
              </div>
            </div>
          ))}

          <div className="rounded-xl p-4" style={{ background: "rgba(27,67,50,0.05)", border: "1px solid rgba(27,67,50,0.12)" }}>
            <div className="flex items-center gap-2 mb-1"><Shield size={14} style={{ color: "var(--accent)" }} /><h3 className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>How It Works</h3></div>
          ...
          {notifs.map(n => (
            <div key={n.id} className="rounded-xl p-4 flex items-center justify-between" style={card}>
              ...
              <button onClick={() => setNotifs(prev => prev.map(x => x.id === n.id ? { ...x, on: !x.on } : x))}
                className="w-9 h-5 rounded-full relative transition-colors" style={{ background: n.on ? "var(--accent-hover)" : "rgba(0,0,0,0.1)" }}>
      ...
      {tab === "account" && (
        <div className="space-y-4 max-w-md">
          <div className="rounded-xl p-4 flex items-center gap-3" style={card}>
            <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold" style={{ background: "rgba(27,67,50,0.12)", color: "var(--accent)" }}>ST</div>
            <div><p className="font-semibold text-sm" style={{ color: "var(--text-primary)" }}>Saint Thomas Capital Partners</p><p className="text-[10px]" style={{ color: "var(--text-secondary)" }}>Signal Engine — Internal</p></div>
          </div>
          <div className="rounded-xl p-4 space-y-3" style={card}>
            <div>
              <label className="text-[10px] uppercase block mb-1" style={{ color: "var(--text-secondary)" }}>Firm Name</label>
              <input defaultValue="Saint Thomas Capital Partners" className="w-full px-3 py-2 rounded-lg text-sm outline-none"
                style={{ background: "var(--bg-surface-alt)", color: "var(--text-primary)", border: "1px solid rgba(0,0,0,0.07)" }} />
            </div>
            <div>
              <label className="text-[10px] uppercase block mb-1" style={{ color: "var(--text-secondary)" }}>Email</label>
              <input defaultValue="deals@stc-capital.com" className="w-full px-3 py-2 rounded-lg text-sm outline-none"
                style={{ background: "var(--bg-surface-alt)", color: "var(--text-primary)", border: "1px solid rgba(0,0,0,0.07)" }} />
            </div>
            <div>
              <label className="text-[10px] uppercase block mb-1" style={{ color: "var(--text-secondary)" }}>Outreach Signature</label>
              <textarea defaultValue="Thomas Saint\nManaging Partner\nSaint Thomas Capital Partners\n(212) 555-0147" rows={3}
                className="w-full px-3 py-2 rounded-lg text-sm outline-none resize-none"
                style={{ background: "var(--bg-surface-alt)", color: "var(--text-primary)", border: "1px solid rgba(0,0,0,0.07)" }} />
            </div>
          </div>
          <button className="w-full py-2 rounded-lg text-sm font-semibold" style={{ background: "var(--accent)", color: "#FFFFFF" }}>Save Changes</button>
        </div>
      )}
    </div>
  );
