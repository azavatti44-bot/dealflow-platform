...
function ScoreRing({ score, size = 56 }: { score: number; size?: number }) {
  ...
  const col = score >= 80 ? "#6EE7B7" : score >= 65 ? "#34D399" : score >= 50 ? "#94A3B8" : "#64748B";
  return (
    <div className="relative inline-flex items-center justify-center">
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={size/2} cy={size/2} r={size/2-4} fill="none" stroke="rgba(148,163,184,0.1)" strokeWidth={3} />
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 space-y-6">
      <div>
        ...
        <p className="text-xs mt-1" style={{ color: "#94A3B8" }}>Query SEC EDGAR, CourtListener, USPTO, and OpenCorporates in real-time</p>
      </div>
      ...
      <form onSubmit={e => { e.preventDefault(); if (query) go(query); }} className="flex gap-3">
        ...
        <button type="submit" disabled={loading}
          className="px-5 py-2.5 rounded-lg text-sm font-semibold disabled:opacity-50" style={{ background: "#6EE7B7", color: "#0B1A0E" }}>
      ...
      <div className="flex flex-wrap gap-2">
        {quick.map(q => (
          <button key={q.t} onClick={() => go(q.n, q.t, q.c)}
            ...
            style={{ background: "#1A351F", color: "#6EE7B7", border: "1px solid rgba(148,163,184,0.12)" }}>
            {q.t}
          </button>
        ))}
      </div>
      ...
      {loading && (
        <div className="flex items-center justify-center py-8 gap-2 text-xs" style={{ color: "#94A3B8" }}>
      ...
      {result && !loading && (
        <div className="space-y-4">
          <div className="rounded-xl p-5" style={cardBg}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1 text-[10px] font-mono" style={{ color: "#64748B" }}>
                    ...
                    <span className="px-2 py-0.5 rounded-full font-bold"
                      style={{ background: result.tier === "hot" ? "rgba(110,231,183,0.15)" : "rgba(148,163,184,0.06)", color: result.tier === "hot" ? "#6EE7B7" : "#94A3B8" }}>
              ...
              <button onClick={() => navigate(`/company/${encodeURIComponent(result.name)}`)}
                className="px-4 py-2 rounded-lg text-xs font-semibold" style={{ background: "#6EE7B7", color: "#0B1A0E" }}>
                View Full Dossier
              </button>
            </div>
          </div>
            ...
            <div className="rounded-xl p-4" style={cardBg}>
              ...
              <div className="space-y-2">
                {result.signals.map((s, i) => (
                  <div key={i} className="flex items-start gap-3 py-2" style={{ borderBottom: i < result.signals.length - 1 ? "1px solid rgba(148,163,184,0.06)" : "none" }}>
          ...
          {result.api_errors.length > 0 && (
            <div className="rounded-lg p-3 text-[10px]" style={{ background: "rgba(110,231,183,0.05)", border: "1px solid rgba(110,231,183,0.15)", color: "#94A3B8" }}>
              <p style={{ color: "#6EE7B7" }}>Some data sources unavailable:</p>
      ...
      {!loading && !result && companies.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: "#94A3B8" }}>Recently Searched</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {companies.slice(0, 6).map((c, i) => (
              <div key={i} onClick={() => navigate(`/company/${encodeURIComponent(c.name)}`)}
                ...
                style={{ background: "#132A1A", border: "1px solid rgba(148,163,184,0.1)" }}>
                <div className="flex items-center justify-between">
                  <span className="font-serif text-sm font-semibold" style={{ color: "#F5F0E6" }}>{c.name}</span>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                    style={{ background: c.tier === "hot" ? "rgba(110,231,183,0.12)" : "rgba(148,163,184,0.06)", color: c.tier === "hot" ? "#6EE7B7" : "#94A3B8" }}>
                    {c.tier} {c.score}
                  </span>
                </div>
      ...
      {!loading && !result && companies.length === 0 && (
        <div className="text-center py-16">
          ...
          <h3 className="font-serif text-lg" style={{ color: "#94A3B8" }}>Search any public company</h3>
