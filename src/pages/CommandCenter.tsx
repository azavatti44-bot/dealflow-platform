...
function ScoreRing({ score, size = 56 }: { score: number; size?: number }) {
  ...
  const col = score >= 80 ? "#16A34A" : score >= 65 ? "#22C55E" : score >= 50 ? "#64748B" : "#64748B";
  return (
    <div className="relative inline-flex items-center justify-center">
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={size/2} cy={size/2} r={size/2-4} fill="none" stroke="rgba(0,0,0,0.07)" strokeWidth={3} />
...
export default function CommandCenter() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 space-y-6">
      <div>
        <h1 className="font-serif text-3xl font-bold" style={{ color: "#1A2416" }}>Signal Command Center</h1>
        <p className="text-xs mt-1" style={{ color: "#64748B" }}>Query SEC EDGAR, CourtListener, USPTO, and OpenCorporates in real-time</p>
      </div>

      <form onSubmit={e => { e.preventDefault(); if (query) go(query); }} className="flex gap-3">
        <div className="flex-1 relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2" size={16} style={{ color: "#64748B" }} />
        ...
        <button type="submit" disabled={loading}
          className="px-5 py-2.5 rounded-lg text-sm font-semibold disabled:opacity-50" style={{ background: "#16A34A", color: "#FFFFFF" }}>
      ...
      <div className="flex flex-wrap gap-2">
        {quick.map(q => (
          <button key={q.t} onClick={() => go(q.n, q.t, q.c)}
            ...
            style={{ background: "#F0EDE5", color: "#16A34A", border: "1px solid rgba(0,0,0,0.08)" }}>
            {q.t}
          </button>
        ))}
      </div>
      ...
      {loading && (
        <div className="flex items-center justify-center py-8 gap-2 text-xs" style={{ color: "#64748B" }}>
        ...
        <div className="space-y-4">
          <div className="rounded-xl p-5" style={cardBg}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="font-serif text-xl font-semibold" style={{ color: "#1A2416" }}>{result.name}</h2>
                  <div className="flex items-center gap-3 mt-1 text-[10px] font-mono" style={{ color: "#64748B" }}>
                    ...
                    <span className="px-2 py-0.5 rounded-full font-bold"
                      style={{ background: result.tier === "hot" ? "rgba(22,163,74,0.12)" : "rgba(0,0,0,0.04)", color: result.tier === "hot" ? "#16A34A" : "#64748B" }}>
              ...
              <button onClick={() => navigate(`/company/${encodeURIComponent(result.name)}`)}
                className="px-4 py-2 rounded-lg text-xs font-semibold" style={{ background: "#16A34A", color: "#FFFFFF" }}>
                View Full Dossier
              </button>
            </div>
          </div>
          ...
          {result.signals.length > 0 && (
            <div className="rounded-xl p-4" style={cardBg}>
              <h3 className="text-sm font-semibold mb-3" style={{ color: "#1A2416" }}>Detected Signals ({result.signals.length})</h3>
              <div className="space-y-2">
                {result.signals.map((s, i) => (
                  <div key={i} className="flex items-start gap-3 py-2" style={{ borderBottom: i < result.signals.length - 1 ? "1px solid rgba(0,0,0,0.04)" : "none" }}>
          ...
          {result.api_errors.length > 0 && (
            <div className="rounded-lg p-3 text-[10px]" style={{ background: "rgba(22,163,74,0.05)", border: "1px solid rgba(22,163,74,0.12)", color: "#64748B" }}>
              <p style={{ color: "#16A34A" }}>Some data sources unavailable:</p>
      ...
      {!loading && !result && companies.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: "#64748B" }}>Recently Searched</h3>
            ...
            {companies.slice(0, 6).map((c, i) => (
              <div key={i} onClick={() => navigate(`/company/${encodeURIComponent(c.name)}`)}
                ...
                <div className="flex items-center justify-between">
                  <span className="font-serif text-sm font-semibold" style={{ color: "#1A2416" }}>{c.name}</span>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                    style={{ background: c.tier === "hot" ? "rgba(22,163,74,0.1)" : "rgba(0,0,0,0.04)", color: c.tier === "hot" ? "#16A34A" : "#64748B" }}>
                    {c.tier} {c.score}
                  </span>
                </div>
      ...
      {!loading && !result && companies.length === 0 && (
        <div className="text-center py-16">
          <Activity className="mx-auto mb-3" size={36} style={{ color: "#64748B" }} />
          <h3 className="font-serif text-lg" style={{ color: "#64748B" }}>Search any public company</h3>
          <p className="text-xs mt-1" style={{ color: "#64748B" }}>Click a ticker above or type a company name to begin signal detection.</p>
        </div>
      )}
    </div>
  );
